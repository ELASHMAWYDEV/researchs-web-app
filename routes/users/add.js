...
router.post("/", async (req, res) => {
  try {
    ...
    //Password validation
    if (user.password.length < 6) errors.push("يجب أن تكون كلمة المرور أكثر من 6 أحرف")
    if (user.password != user.passwordConfirm) errors.push("يجب أن يكون كلمة المرور وتأكيد كلمة المرور متطابقين");

    //Check if username exist in DB
    const usernameCheck = await db.collection("users").findOne({ username: user.username });
